...
/* 댓글 */
//review/replies/list/<int:post_id>/<int:page>
let page = 1;
const replyService = (function(){
    async function getList(){
        const response = await fetch(`/lesson/review/replies/list/${post_id}/${page}`);
        const replies = await response.json()
        return replies;
    }

    async function write(reply_content){
        const response = await fetch("/lesson/review/replies/write/", {
            method: 'post',
    ...
    async function modify(id, reply_content){
        const response = await fetch("/lesson/review/replies/modify/", {
            method: 'post',
            headers: {'Content-Type': 'application/json; charset=utf-8'},
    ...
    async function remove(id){
        const response = await fetch(`/lesson/review/replies/delete/${id}`, {
            method: 'get',
            headers: {'Content-Type': 'application/json; charset=utf-8'},
        });
        // response.catch((error) => {})
...
$(".heart").each((i, heart) => {
  $(heart).on('click', () => {
    ...
    if(i===0){
      ...
      $(".heart").eq(0).addClass("active");
      fetch("/lesson/review/likes/delete/", {
            method: 'post',
            headers: {'Content-Type': 'application/json; charset=utf-8'},
            body: JSON.stringify({id: post_id})
      }).then(()=>getLikeCount())
...
function getLikeCount(){
    let id = post_id;
    fetch(`/lesson/review/likes/count/${id}`, {
            method: 'get',
            headers: {'Content-Type': 'application/json; charset=utf-8'},
...
function checkLike(){
    ...
    fetch(`/lesson/review/likes/exist/${post_id}`, {
        method: 'get',
        headers: {'Content-Type': 'application/json; charset=utf-8'},
    }).then((response)=> response.json())
        .then((check) => {
