...
$(".sub-header-item-content").eq(2).addClass("sub-header-item-active");
...
const replyService = (function(){
    async function getList(){
        const response = await fetch(`/helpers/replies/list/${post_id}/${page}`);
        const replies = await response.json()
        return replies;
    }
    async function write(reply_content){
        const response = await fetch("/helpers/replies/write/", {
            method: 'post',
    ...
    async function modify(id, reply_content){
        const response = await fetch("/helpers/replies/modify/", {
            method: 'post',
            headers: {'Content-Type': 'application/json; charset=utf-8'},
    ...
    async function remove(id){
        const response = await fetch(`/helpers/replies/delete/${id}`, {
            method: 'get',
            headers: {'Content-Type': 'application/json; charset=utf-8'},
        });
        // response.catch((error) => {})
                        ...
                        <div style="display: none" class="modify-form">
                              <div style="padding: 0;" class="comment-textarea-wrapper">
                                ...
                                <div class="comment-textarea-button-wrapper">
                                  ...
                                  <button onclick="modifyReply(this)" class="write-button" type="button">등록</button>
                                  <input value="${reply.id}" type="hidden">
                                </div>
                              </div>
                          </div>
...
function checkLike(){
    ...
    fetch(`/helpers/likes/exist/${post_id}`, {
        method: 'get',
        headers: {'Content-Type': 'application/json; charset=utf-8'},
    }).then((response)=> response.json())
        .then((check) => {
